...
const LrBiltyEdit = ({ navigation, route }) => {
  const { item } = route.params;

  const [oldlr, setoldlr] = React.useState([])
  const [editing, setediting] = React.useState(false)
  const [basicform, setbasicform] = React.useState({
    biltynumber: item.basicform.biltynumber,
    date: item.basicform.date,

    consignorname: item.basicform.consignorname,
    addressfrom: item.basicform.addressfrom,
    addressto: item.basicform.addressto,
    lorrynumber: item.basicform.lorrynumber,

  })

  const [costform, setcostform] = React.useState({
    packingchargerate: item.costform.packingchargerate,
    packingchargepaid: item.costform.packingchargepaid,
    unpackingchargerate: item.costform.unpackingchargerate,
    unpackingchargepaid: item.costform.unpackingchargepaid,

    loadingchargerate: item.costform.loadingchargerate,
    loadingchargepaid: item.costform.loadingchargepaid,

    unloadingchargerate: item.costform.unloadingchargerate,
    unloadingchargepaid: item.costform.unloadingchargepaid,

    freightchargesrate: item.costform.freightchargesrate,
    freightchargespaid: item.costform.freightchargespaid,
    grchargerate: item.costform.grchargerate,
    grchargepaid: item.costform.grchargepaid,
    insurancechargesrate: item.costform.insurancechargesrate,
    insurancechargespaid: item.costform.insurancechargespaid,
    ...
    totalamount: item.getotal,
    totalamountpaid: item.gettotalpaid,
  })

  const [descriptionform, setdescriptionform] = React.useState({
    householditems: item.descriptionform.householditems,
    householditemswtactual: item.descriptionform.householditemswtactual,
    householditemswtcharged: item.descriptionform.householditemswtcharged,
    officeitems: item.descriptionform.officeitems,
    officeitemswtactual: item.descriptionform.officeitemswtactual,
    officeitemswtcharged: item.descriptionform.officeitemswtcharged,
    industrialitems: item.descriptionform.industrialitems,
    industrialitemswtactual: item.descriptionform.industrialitemswtactual,
    industrialitemswtcharged: item.descriptionform.industrialitemswtcharged,
    cartransportation: item.descriptionform.cartransportation,
    cartransportationwtactual: item.descriptionform.cartransportationwtactual,
    cartransportationwtcharged: item.descriptionform.cartransportationwtcharged,
    biketransportation: item.descriptionform.biketransportation,
    biketransportationwtactual: item.descriptionform.biketransportationwtactual,
    biketransportationwtcharged: item.descriptionform.biketransportationwtcharged,
    asperlistattached: item.descriptionform.asperlistattached,
    asperlistattachedwtactual: item.descriptionform.asperlistattachedwtactual,
    asperlistattachedwtcharged: item.descriptionform.asperlistattachedwtcharged,
  })
    ...
    AsyncStorage.getItem('token')
      .then(token => {
        ...
        fetch(`${envs.BACKEND_URL}/updatealldocs`, {
          ...
          .then(data => {
            console.log(data)
            if (data.message == "LR Bilty Updated Successfully") {
              alert("LR Bilty Updated Successfully")
              setediting(false)
              // resetvalues()
            }
            else {
  ...
  const showprintablerbill = async () => {
    let userid = 0;
    let docid = item.basicform.biltynumber;
    let doctype = 'lrbilty'
    AsyncStorage.getItem('token')
      .then(token => {
          ...
          <ScrollView>
            <View style={sformcontainer}>
              ...
              <View style={sformcontainerin}>
                ...
                <TextInput style={sforminput} value={basicform?.lorrynumber} onChangeText={(text) => setbasicform({ ...basicform, lorrynumber: text })} />
              </View>
            </View>
            ...
            >
              ...
              <View style={sformcontainerin2}>
                <Text style={sformlabel}>Weight Actual</Text>
              ...
              <View style={sformcontainerin2}>
                <Text style={sformlabel}>Weight Charge</Text>
